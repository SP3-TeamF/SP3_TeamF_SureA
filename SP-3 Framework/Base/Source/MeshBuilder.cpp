...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile){
	...
	//Check weather no. of slices is within 1-256
	slices = Math::Clamp(slices, 1, 256);

	//other code instead of clamp (slices < 1) ? (slices = 1) : (slices > 256) ? (slices = 256) : (slices);
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char>& heightMap){
	...
	for (int z = 0; z < terrainSize; z++)
	{
		for (int x = 0; x < terrainSize; x++)
		{
			...
			v.texCoord.Set((float)x / terrainSize * 8, 1.0f - (float)z / terrainSize * 8);//Repeat texture 8 times
			vertex_buffer_data.push_back(v);

			if (z + 2 < terrainSize -1 && x + 2 < terrainSize - 1)
			{
				scaledHeight = (float)heightMap[(z + 2) * terrainSize + x] / SCALE_FACTOR;
				Vector3 vertexZ((static_cast<float>(x) / terrainSize - 0.5f, scaledHeight, static_cast<float>(z + 2) / terrainSize - 0.5f));

				scaledHeight = (float)heightMap[z * terrainSize + x + 2] / SCALE_FACTOR;
				Vector3 vertexX((static_cast<float>(x + 2) / terrainSize - 0.5f, scaledHeight, static_cast<float>(z) / terrainSize - 0.5f));

				vertexZ = Vector3(v.pos.x, v.pos.y, v.pos.z) - vertexZ;
				vertexX = Vector3(v.pos.x, v.pos.y, v.pos.z) - vertexX;

				Vector3 normal = vertexZ.Cross(vertexX).Normalize();
...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color){
	...
	{
		float posX = cos(Math::DegreeToRadian(i * anglePerSlice)) * 0.5;
		float posY = sin(Math::DegreeToRadian(i * anglePerSlice)) * 0.5;

		v.pos.Set(posX, posY, 0);
		v.color = color;
		v.texCoord.Set(posX, posY);
		vertex_buffer_data.push_back(v);
...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateCircleMinimap(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (int i = 0; i < numSlices; ++i)
	{
		float posX = cos(Math::DegreeToRadian(i * anglePerSlice)) * 0.5 * length;
		float posY = sin(Math::DegreeToRadian(i * anglePerSlice)) * 0.5 * length;
...
Mesh* MeshBuilder::GenerateSquareMinimap(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateTileMesh(const std::string &meshName, unsigned int numRow, unsigned int numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
